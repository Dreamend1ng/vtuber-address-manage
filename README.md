...
| 版本更新（可选） | 解锁后检查上游新版本，弹窗提示；配置更新 Token 后一键把代码更新到最新，只同步代码、保留你的定制，未配置时给出手动同步指引 |
...
- 仓库根目录新增 `version.json` 版本标记，应用解锁后与上游对比（读取失败时回退到 GitHub Releases）；
- 「设置 → 版本更新」新增配置面板：开关更新检查、填写上游与自己站点仓库、配置更新 Token，并提供 Token 创建链接与权限说明；
- 初始化向导新增「更新设置」步骤，可跳过；
- 发现新版本时弹窗显示「当前版本 → 最新版本」并可查看更新说明；配置了更新 Token 可**一键更新**，未配置时给出 Fork 的 Sync fork 与 upstream 拉取两种手动同步指引；
- 一键更新只同步有变化的代码文件并提交一次，由 Actions 自动重新部署；跳过 `docs/`、README、TUTORIAL 以及你改过的 `src/config.ts` 与网站图标；
- 更新 Token 只授权站点仓库的 Contents 读写，单独保存于本机保险库，不进入任何分享链接。
